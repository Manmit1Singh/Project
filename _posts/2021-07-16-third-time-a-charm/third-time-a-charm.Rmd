...
d. Key takeways from the above review include:-
...
The files provided (car-assignements, cc_data, loyalty_data and gps) are imported. One other file was created in excel for data exploration to understand if it was a useful format to have the data in for creating visuals. The files is All_Combined4. Specifically, All_Combined4 merges the cc_data and loyalty_card data in the following table form “Date, Location, Price, CC.Loyalty". For avoidance of doubt, the file was not used to answer the MC-2 question.
...
The steps below flesh out the data cleaning process.For each data set, glimpse("data set name") was used to identify the format of the data.Thereafter, the following was performed :- 
...
# 5. Looking at the cleaned data sets
glimpse(loyalty_clean)
glimpse(credit_card_clean)
glimpse(gps_clean)
glimpse(combined_trans_pivot) #used for exploration, not part of the final solution
```


d. Merging Data sets
Given that CarID is tagged to employee name, GPS data is merged with Car Assignment data as follows:
...
### Time to get a sense of what was happening!!!
...
check_outlierS <- credit_card_clean %>%
   ...
   labs(
    ...
    y = "Price",
    title = "Boxplot of Credit Card vs Price by Location")+
...
- 9152
![](5.01.jpg)
Figure 4.01: Datatable view of cc9152
...
- 7792
![](5.02.jpg)
Figure 4.02: Datatable view of cc7792
...
![](5.03.jpg)
Figure 4.03: Datatable view of cc6816
...
-	8332 
![](5.04.jpg)
Figure 4.04: Datatable view of cc8332
...
Figure 4.05: Datatable view of cc spend at the airport

8642 went to the airport between the 6th -9th and 13th – 16th, spending twice each day between 12-1pm and 3-5pm.

-	Abila Scrapyard
![](5.06.jpg)
Figure 4.06: Datatable view of cc spent at Abila Scrapyard
...
![](5.07.jpg)
Figure 4.07: Datatable view of cc2276
...
Figure 4.08: Datatable view of Albert's Fine Clothing
...
Figure 4.09: Datatable view of cc spend at Chostus hotel

Two employees with cc numbers 2540 and 9683 spent almost within half an hour to each other. An oulier – 5010- spent on 18th Jan at 12pm, a day after 9683 and 2540. 


- Frydos Autosupply n More 
![](5.10.jpg)

Figure 4.10: Datatable view of cc spend at Frydos
...
- Nationwide Refinery: Spending pattern seems to have a rhythm. In the first week, the time gaps seem consistent with larger spends. In the second week, we spot a similar rhythm, albeit with smaller spends.
...
-	4530 spend pattern at Kronos Pipe and Irrigation and Maximum Iron: The card owner has the same spending pattern across the two, starting high on the 7th, before dipping low on the 9th and increasing on the 15th.

-	8642 at Abila Airport: The card owner exhibits at ‘W’ spending pattern at the airport. Spending twice a day at times.
...
The usage count is plotted against the loyalty number in a bar chart to understand which card users were the most active.
...
check_outliers__lylty <- loyalty_clean %>%
   ...
   labs(
    x = "Loyalty Card",
    y = "Price",
    title = "Boxplot of Loyalty Card by Price by Location")
    theme(panel.border = element_rect(colour="grey60", fill = NA),
          axis.text.x.bottom = element_blank())
...
![](6.01.jpg)
Figure 5.01: Datatable view of loyalty card spend at Abila Airport



Credit Card Data table: 
![](6.02.jpg)
Figure 5.02: Datatable view of cc spend at Abila Airport
...
![](6.03.jpg)
Figure 5.03: Datatable view of L5756 loyalty card spend 
...
Figure 5.04: Datatable view of L4063 loyalty card spend
...
![](6.06.jpg)
Figure 5.05: Datatable view of loyalty card spend at Carlyle Chemical


Comparing this to the credit card data: 
![](6.05.jpg)
Figure 5.06: Datatable view of cc spend at Carlyle Chemical
...
By way of exploration, the following code chunk was used to plot the linear relationship between the credit card and loyalty card spend. However, to answer the question posted, another code chuck will be used in section 9.
...
From the above exploration, it is possible to zoom in on the locations and use visual analysis to spot that not all credit cards and loyalty amount match. Using this, it is possible to infer to some extent the credit card owners and loyalty card. 
...
![](7.01.jpg)
Figure 6.01: Identifying travel patterns using gps data Name vs Timestamp 



1.	Sten/President and CEO was away for a long period and only started being tracked after January 13.

2.	Nils/ IT Helpdesk, Loreto/ Site Control and Isia/Perimeter Control traveled at an odd timing given that the rest of the employees did not do this. This should be investigated further for time of travel and locations.
...
![](7.02.jpg)
Figure 6.02: Identifying time lag travel patterns using gps data Name vs Timestamp 
...
Figure 6.03: Map of Lucas Alcazar's travel pattern using gps
...
Figure 6.04: Map of Kare Orilla's travel pattern using gps
...
![](7.05.jpg)
Figure 6.05: Map of Linnea's travel pattern using gps
...
![](7.06.jpg)
Figure 6.06: Map of Orhan's travel pattern using gps
...
![](7.07.jpg)
Figure 6.07: Map of Edvard's travel pattern using gps
...
![](7.08.jpg)
Figure 6.08: Map of Isia's travel pattern using gps


![](7.09.jpg)
Figure 6.09: Map of Betrand's travel pattern using gps
...
Figure 6.10: Map of Axel's travel pattern using gps



![](7.11.jpg)

Figure 6.11: Map of Frente's travel pattern using gps
...
Figure 7.01: Zoom in on relationship of nodes for Chostus Hotel
...
![](8.02.jpg)
Figure 7.02: Zoom in on relationship of nodes for Airport and Scrapyard


The employee who visited the scrapyard, 2276, also visited the airport. Both these locaions have contentious spending. It is useful to note the other employees who visited the airport as well, as they all may be connected (cc 5642, 9220, 9614 and 3506).
...
![](8.03.jpg)
Figure 7.03: Zoom in on relationship of nodes for Golf Course


cc 8332, 7688, 8156, 2463 and 5010 are linked to the Golf Course and may have a relationship. Given the contentious activity at the hotel, all the employees who hold these credit cards should be questioned. 
...
Boxplots were used to check for anomalies. The boxplots were plotted for cc number against price. From this, the following anomalies were detected.
...
9735| *Pattern:* Daily spending at Nationwide Refinery before spending at Stewart’s. *Timing:* Consistently - ~10am at the refinery and ~1130 at the Stewart’s *Amount:* Between ~ $600 - $4000 Daily expenditure that has a larger amount variation and consistency of spending|Big variation and frequency of spending| Nationwide Refinery, Stewart and Son’s Fabrication|
...
2540, 9683 and 5010| *Pattern:* Spent almost within half an hour to each other in the early afternoon between ~1240 – ~1315 An outlier expenditure – 5010- spend on 18th Jan (Saturday) at 12pm, a day after 9683 and 2540.| Consistency of the time difference in expenditure between 2 persons|Chostus Hotel|
...
-	Expenditure: Cc expenditure is higher than loyalty card at some places. Some places where expenditure occurred, had no gps data, for example, the airport.
...
The method does have some drawbacks. For example, the loyalty card has no time stamp. On that note, it is not entirely accurate to match the prices based on location. Some employees may have bought items without using a credit card but used the loyalty card. 
...
l_cc <- ggplot(loyalty_cc_merge) +
  ...
  labs(
    x = "Credit Card",
    y = "Loyalty Card",
    title = "Matching of Credit Card (x-axis) to Loyalty Card Number (y-axis") +
  theme_minimal() +
...
Improvements could be done specifically for displaying the graphs. Getting the fit of each chart and picture was challenging. Patchwork library could be used in the future. In addition, using push force for nodes display.

In addition to the challenge of displaying charts nicely, the user is unable to smoothly interact with the visuals. It could be better to wrap the project in a Shiny application. This will be done in the following part of the project.

On reflection of using R for visualisation, the challenge for creating the visual and rendering on a webpage is getting the size of the pictures and charts right. In addition, looking for the appropriate code chunks to use. The internet is filled with many approaches to using R and packages, but not all are appropriate or usable in every scenario. The formatting of text in Rmd is also not as intuitive, for example, creating of tables quikcly and aligning chapters and paragraphs. To that end, practice is the only way and constantly working on R to hone the skill and build the knowledge.

This project has been extremely useful to ramp up learning coding for data visualisation in R.
